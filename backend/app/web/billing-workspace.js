...
async function billingQuickImportCsv(){
  ...
  const started=showFusaaOperation("Importation des produits…");try{const result=await api("/api/v1/billing/products/import?organization_id="+encodeURIComponent(org),{method:"POST",body});tell(result.created+" produit(s) importé(s) pour la facturation.");await billingProductSearch(true)}catch(error){tell(error.message)}finally{await hideFusaaOperation(started)}
}
...
async function billingImportCsv(){const file=document.getElementById("billCsvFile")?.files[0];if(!file){tell("Choisissez un fichier CSV.");return}const body=new FormData();body.append("file",file);const started=showFusaaOperation("Importation des produits…");try{const result=await api("/api/v1/billing/products/import?organization_id="+encodeURIComponent(org),{method:"POST",body});tell(result.created+" produit(s) importé(s)"+(result.errors.length?" · "+result.errors.length+" ligne(s) ignorée(s)":""));billingClosePopup();await billingOpen("dashboard")}catch(error){tell(error.message)}finally{await hideFusaaOperation(started)}}
