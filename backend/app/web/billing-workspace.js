...
function billingFacturationAssistantOpen(prefill=""){
  ...
  if(billingRoot&&dialog.parentElement!==billingRoot)billingRoot.appendChild(dialog);
  const suggestions=dialog.querySelector(".billing-assistant-suggestions");
  dialog.insertBefore(suggestions,document.getElementById("billingAssistantMessages"));
  const suggestionButtons=suggestions.querySelectorAll("button");
  suggestionButtons[0].onclick=()=>{billingState.assistantDocumentType="QUOTE";billingFacturationAssistantMessage("assistant","Créer un devis","Collez votre liste, un produit par ligne. La quantité est facultative : « Ramette A4 » ajoute 1 unité, « 2 x Ramette A4 » en ajoute 2.");document.getElementById("billingAssistantInput").focus()};
  suggestionButtons[1].onclick=()=>{const input=document.getElementById("billingAssistantInput");input.focus();input.placeholder="Un produit par ligne. Exemple :\nRamette A4\n3 x Cahier"};
...
function billingReviewAssistantProducts(draft){
  // Only explicit confirmation writes missing products, using the existing
  // billing endpoint and its administrator permission checks.
  return new Promise(resolve=>{
    const dialog=document.createElement("dialog");dialog.className="billing-assistant-products billing-quick-dialog billing-reference-dialog";
    dialog.setAttribute("aria-label","Confirmer les nouveaux produits et leurs prix");
    dialog.innerHTML='<div class="billing-dialog-heading"><h2>Produits et prix</h2><button type="button" class="secondary" data-cancel aria-label="Fermer">×</button></div><p>Vérifiez les désignations. Indiquez le prix des nouveaux produits : ils seront ajoutés au catalogue de facturation après confirmation.</p><p>Un produit par ligne, dans l’ordre de votre liste. Si plusieurs noms sont regroupés, utilisez « Corriger la liste ».</p><form><div class="billing-assistant-product-list">'+draft.lines.map((line,index)=>{
      const existing=Boolean(line.product_id),candidates=line.candidates||[];
      return '<div class="billing-assistant-product-row" data-index="'+index+'"><b>'+(index+1)+'</b><label>Produit '+(existing?'· trouvé':'· à confirmer')+'<input name="name" required maxlength="160" value="'+esc(line.description)+'" '+(existing?'readonly':'')+'></label><label>Quantité<input name="quantity" type="number" required min="0.01" max="9999" step="any" value="'+esc(line.quantity)+'"></label><label>Prix unitaire FCFA<input name="price" type="number" required min="0" step="0.01" value="'+esc(line.unit_amount??'')+'" '+(existing?'readonly':'')+' placeholder="Prix à renseigner"></label>'+(candidates.length?'<label class="billing-product-candidates">Correspondances<select name="candidate" required><option value="">Choisir le produit existant</option>'+candidates.map(p=>'<option value="'+esc(p.id)+'">'+esc(p.name)+' · '+esc(p.source)+' · '+billingCurrency(p.price_xof)+'</option>').join('')+'</select></label>':'')+'</div>';
    }).join('')+'</div><p class="billing-import-error" role="alert" data-error></p><div class="billing-actions"><button type="button" class="secondary" data-cancel>Corriger la liste</button><button type="submit">Confirmer les prix et préparer la facture</button></div></form>';
    (document.getElementById("billingWorkspace")||document.getElementById("billing")).appendChild(dialog);
    ...
    dialog.querySelectorAll('[data-cancel]').forEach(button=>button.onclick=()=>{if(!saving)finish(null)});
    dialog.addEventListener('cancel',event=>{event.preventDefault();if(!saving)finish(null)});
    dialog.addEventListener('close',()=>{if(!finished){finished=true;dialog.remove();resolve(null)}});
    dialog.querySelectorAll('[name="candidate"]').forEach(select=>select.onchange=()=>{
      const row=select.closest('[data-index]'),line=draft.lines[Number(row.dataset.index)],product=line.candidates.find(p=>p.id===select.value);
      const price=row.querySelector('[name="price"]'),name=row.querySelector('[name="name"]');
      if(product){name.value=product.name;price.value=product.price_xof;name.readOnly=true;price.readOnly=true}
    });
    dialog.querySelector('form').onsubmit=async event=>{
      event.preventDefault();if(saving)return;
      const form=event.currentTarget;if(!form.reportValidity())return;
      const rows=[...form.querySelectorAll('[data-index]')],error=form.querySelector('[data-error]'),submit=form.querySelector('[type="submit"]');
      error.textContent='';saving=true;submit.disabled=true;dialog.querySelectorAll('[data-cancel]').forEach(b=>b.disabled=true);
      try{
        // Refresh once before creation to catch products already added in
        // another popup. Do not republish private products in the shop.
        const catalogue=await api('/api/v1/billing/products?organization_id='+encodeURIComponent(org));
        const products=catalogue.items||[],key=name=>name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().replace(/\s+/g,' ').toLocaleLowerCase('fr');
        for(let index=0;index<rows.length;index++){
          const row=rows[index],line=draft.lines[index],name=row.querySelector('[name="name"]').value.trim(),quantity=Number(row.querySelector('[name="quantity"]').value),price=Number(row.querySelector('[name="price"]').value);
          if(!name||!Number.isFinite(price)||price<0||!Number.isFinite(quantity)||quantity<=0)throw Error('Vérifiez le produit et le prix de la ligne '+(index+1));
          submit.textContent='Enregistrement '+(index+1)+' / '+rows.length+'…';
          const candidateId=row.querySelector('[name="candidate"]')?.value;
          let product=line.product_id?products.find(p=>p.id===line.product_id):candidateId?products.find(p=>p.id===candidateId):null;
          if(!product){
            const matches=products.filter(p=>key(p.name)===key(name));
            if(matches.length>1)throw Error('Plusieurs produits portent le nom « '+name+' ». Corrigez la liste puis choisissez la correspondance.');
            product=matches[0];
          }
          if(!product){
            const saved=await api('/api/v1/billing/products',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({organization_id:org,name,unit_price:price,unit:line.unit||'piece'})});
            product={...saved,price_xof:price,unit:line.unit||'piece'};products.push(product);
          }
          // Save each successful row for safe retry if a later request fails.
          line.product_id=product.id;line.description=product.name;line.quantity=quantity;line.unit=product.unit||'piece';line.source=product.source;
          line.unit_amount=product.price_xof;line.candidates=[];
          row.querySelector('[name="name"]').readOnly=true;
          row.querySelector('[name="price"]').value=product.price_xof;row.querySelector('[name="price"]').readOnly=true;
          billingState.productCatalogLoaded=false;
        }
        draft.total_amount=draft.lines.reduce((sum,line)=>sum+line.quantity*line.unit_amount,0);
        finish(draft);
      }catch(e){error.textContent=e.message+' Les lignes déjà enregistrées sont conservées ; vous pouvez réessayer.'}
      finally{saving=false;submit.disabled=false;submit.textContent='Confirmer les prix et préparer la facture';dialog.querySelectorAll('[data-cancel]').forEach(b=>b.disabled=false)}
    };
    dialog.showModal();dialog.querySelector('input[name="price"]:not([readonly])')?.focus();
  });
}
function billingFacturationAssistantMessage(role,title,text,draft){
  const target=document.getElementById("billingAssistantMessages");if(!target)return;
  let draftHtml="";
  if(draft){
    billingState.assistantDraftHistory=billingState.assistantDraftHistory||[];
    const index=billingState.assistantDraftHistory.push(draft)-1;
    const lines=draft.lines.map(line=>'<li>'+esc(line.description)+' · '+esc(line.quantity)+' × '+billingCurrency(line.unit_amount)+'</li>').join("");
    draftHtml='<section class="billing-assistant-draft"><span class="billing-type-badge">'+esc(billingDocumentLabel(draft.document_type))+'</span><strong>Total des lignes : '+billingCurrency(draft.total_amount)+'</strong><ol>'+lines+'</ol><button type="button" onclick="billingFacturationAssistantApplyDraft('+index+')">Appliquer au brouillon</button></section>';
  }
  target.insertAdjacentHTML("beforeend",'<article class="billing-assistant-message '+role+'"><b>'+esc(title)+'</b><p>'+esc(text||"")+'</p>'+draftHtml+'</article>');target.scrollTop=target.scrollHeight;
}
async function billingFacturationAssistantSend(message){
  const text=String(message||"").trim();if(!text||billingState.assistantBusy)return;
  billingState.assistantBusy=true;
  const input=document.getElementById("billingAssistantInput"),messages=document.getElementById("billingAssistantMessages"),button=document.querySelector("#billingAssistantForm button");
  billingFacturationAssistantMessage("user","Vous",text);input.value="";input.disabled=true;button.disabled=true;
  messages.insertAdjacentHTML("beforeend",'<article id="billingAssistantTyping" class="billing-assistant-typing"><i></i><i></i><i></i> Recherche des produits…</article>');messages.scrollTop=messages.scrollHeight;
  try{
    const data=await api("/api/v1/billing/assistant",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({organization_id:org,message:text,...billingFacturationAssistantContext()})});
    document.getElementById("billingAssistantTyping")?.remove();
    let draft=data.draft;
    if(draft&&billingState.assistantDocumentType&&!/devis|proforma|facture|reçu|livraison/i.test(text))draft.document_type=billingState.assistantDocumentType;
    if(data.requires_product_review){
      billingFacturationAssistantMessage("assistant",data.title,data.answer);
      draft=await billingReviewAssistantProducts(draft);
      if(!draft){input.value=text;billingFacturationAssistantMessage("assistant","Liste à compléter","Votre liste est conservée dans la saisie. Séparez chaque produit par un retour à la ligne, puis renvoyez-la.");return}
    }
    if(draft)billingState.assistantDraft=draft;
    billingFacturationAssistantMessage("assistant",draft?"Proposition de document":data.title,draft?"Vos lignes sont prêtes dans l’ordre de la liste. Appliquez-les au brouillon, puis choisissez le client et vérifiez la facture.":data.answer,draft);
  }catch(error){input.value=text;billingFacturationAssistantMessage("assistant","Assistant Facturation",error.message||"La demande n’a pas pu être traitée.")}
