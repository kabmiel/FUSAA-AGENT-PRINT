...
async function billingImportAssistantExecute(){
  ...
  const button=document.getElementById("billingImportExecute"),started=showFusaaOperation("Import Boulangerie en cours…");button.disabled=true;target.innerHTML='<p class="billing-assistant-typing"><i></i><i></i><i></i> Importation sécurisée des données…</p>';const cards=[];
  try{for(const item of entries){const body=new FormData();body.append("file",item.file);body.append("kind",item.kind);const result=await api("/api/v1/billing/import/execute?organization_id="+encodeURIComponent(org),{method:"POST",body});cards.push(billingImportResultCard(result,"done"))}target.innerHTML=cards.join("");await billingLoadReference();tell("Import Boulangerie terminé. Consultez les cartes de résultat avant de fermer.")}catch(error){target.insertAdjacentHTML("beforeend",'<article class="billing-import-result error"><b>Import interrompu</b><p>'+esc(error.message)+'</p></article>');tell(error.message)}finally{button.disabled=false;await hideFusaaOperation(started)}
}
...
async function billingImportHeaders(){const file=document.getElementById("billingHeadersCsv")?.files[0];if(!file)return;const body=new FormData();body.append("file",file);try{const result=await api("/api/v1/billing/headers/import?organization_id="+encodeURIComponent(org),{method:"POST",body});tell(result.created+" entête(s) importée(s)"+(result.errors?.length?" · "+result.errors.length+" ligne(s) ignorée(s)":""));billingHeaders()}catch(error){tell(error.message)}}
