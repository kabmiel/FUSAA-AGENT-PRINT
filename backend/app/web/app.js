...
function selectView(view){const target=$(view);if(!target)return;if(view==="arrivals")loadActivities();if(view==="publicOrders")loadGuestOrders();if(view==="settings"){loadMonitor();loadSystemHealth()}document.querySelectorAll(".view").forEach(item=>item.classList.toggle("active",item===target));document.querySelectorAll("#nav a").forEach(item=>item.classList.toggle("active",item.dataset.view===view));history.replaceState(null,"","#"+view);window.scrollTo({top:0,behavior:"smooth"})}
