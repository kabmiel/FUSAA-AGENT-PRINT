...
function selectView(view){const target=$(view);if(!target)return;if(view==="arrivals")loadActivities();if(view==="production")loadProduction();if(view==="publicOrders"){loadGuestOrders();loadPublicPricing()}if(view==="settings"){loadMonitor();loadSystemHealth()}document.querySelectorAll(".view").forEach(item=>item.classList.toggle("active",item===target));document.querySelectorAll("#nav a").forEach(item=>item.classList.toggle("active",item.dataset.view===view));history.replaceState(null,"","#"+view);window.scrollTo({top:0,behavior:"smooth"})}
...
mountAssistant();mountSettings();mountArrivals();mountProduction();mountGuestOrders();mountArrivalPopup();mountChatStyle();mountStyle();compactNavigation();addExtensionDownload();applyTheme(localStorage.getItem("fusaa-theme")||"dark");mountThemeStyle();mountThemeControls();mountProgressStyle();mountJobStyle();
