...
function selectView(view){const target=$(view);if(!target)return;if(view==="arrivals")loadActivities();if(view==="production")loadProduction();if(view==="team")loadTeam();if(view==="history")loadAuditHistory();if(view==="publicOrders"){loadGuestOrders();loadPublicPricing()}if(view==="settings"){loadMonitor();loadSystemHealth()}document.querySelectorAll(".view").forEach(item=>item.classList.toggle("active",item===target));document.querySelectorAll("#nav a").forEach(item=>item.classList.toggle("active",item.dataset.view===view));history.replaceState(null,"","#"+view);window.scrollTo({top:0,behavior:"smooth"})}
...
async function loadPublicPricing(){try{const pricing=await api("/api/v1/public-pricing");$("publicPriceBase").value=pricing.base||0;$("publicPriceCopy").value=pricing.per_copy||0;$("publicPricePage").value=pricing.per_page||0}catch(error){if($("publicPricingStatus"))$("publicPricingStatus").textContent=error.message}}
async function savePublicPricing(event){event?.preventDefault();try{await api("/api/v1/public-pricing",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({base:Number($("publicPriceBase").value||0),per_copy:Number($("publicPriceCopy").value||0),per_page:Number($("publicPricePage").value||0)})});$("publicPricingStatus").textContent="Tarif enregistré. Actualisez le devis de chaque commande non payée."}catch(error){$("publicPricingStatus").textContent=error.message}}
...
async function addTeamMember(event){event?.preventDefault();const email=$("teamEmail").value.trim(),role=$("teamRole").value;if(!email)return;try{await api("/api/v1/workshops/"+encodeURIComponent(workshop)+"/members",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({user_email:email,role})});$("teamEmail").value="";tell("Membre ajouté à l’atelier.");loadTeam()}catch(error){tell(error.message)}}
...
mountAssistant();mountSettings();mountArrivals();mountProduction();mountGuestOrders();mountTeam();mountAuditHistory();mountArrivalPopup();mountChatStyle();mountStyle();compactNavigation();addExtensionDownload();applyTheme(localStorage.getItem("fusaa-theme")||"dark");mountThemeStyle();mountThemeControls();mountProgressStyle();mountJobStyle();
