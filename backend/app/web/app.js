...
async function loadGuestOrders(){const target=$("guestOrdersList");if(!target||!token)return;try{const orders=await api("/api/v1/guest-orders");target.innerHTML=orders.length?orders.map(order=>{const paid=order.payment_status==="PAID";return '<article class="guest-order '+(paid?'paid':'')+'"><div><span class="job-badge">'+esc(paymentLabel(order.payment_status))+'</span><h3>'+esc(order.order_number)+'</h3><p>'+esc(order.document_name)+' · '+esc(order.display_name||"Client")+' · '+esc(order.phone)+'</p><small>'+esc(order.status)+' · '+money(order.estimated_cost)+(order.payment_reference?' · Réf. '+esc(order.payment_reference):'')+(order.invoice_number?' · Reçu '+esc(order.invoice_number):'')+'</small></div><div class="guest-order-actions">'+(!paid?'<button class="secondary" onclick="refreshGuestQuote(\''+esc(order.id)+'\')">Actualiser devis</button>':'')+'<button class="'+(paid?'secondary':'')+'" onclick="setGuestPayment(\''+esc(order.id)+'\',\''+(paid?'PENDING':'PAID')+'\')">'+(paid?'Retirer validation':'Valider paiement')+'</button>'+ (paid?'<button class="secondary" onclick="createGuestReceipt(\''+esc(order.id)+'\')">Reçu</button>':'')+'<button class="secondary" onclick="openGuestOrderPrint(\''+esc(order.print_job_id)+'\',\''+esc(order.payment_status)+'\')">Impression</button></div></article>'}).join(""):'<p class="muted">Aucune commande publique pour le moment.</p>'}catch(error){target.textContent=error.message}}
