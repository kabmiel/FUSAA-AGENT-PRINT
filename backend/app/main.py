...
def create_billing_competition(invoice_id:str,data:BillingCompetitionIn,user:User=Depends(current_user),db:Session=Depends(get_db)):
    # Same behaviour as Boulangerie: a separate draft quote, issued with a
    # different header and calculated from the source line unit prices.
    original=one(db,Invoice,invoice_id);require_member(db,user,original.organization_id)
...
def index():return HTMLResponse((Path(__file__).parent/"web"/"shop.html").read_text(encoding="utf-8"))
...
def storefront_layout_script()->str:
    css="""
    :root{background:#f6f7f8;color:#111827}.top{height:104px;padding:0 max(24px,calc((100% - 1740px)/2));background:#fff!important;color:#0b1018!important;box-shadow:0 1px 0 #e5e7eb;gap:24px}.brand{font-size:0;color:#0b1018}.brand i{width:48px;height:48px;margin:0;border-radius:15px;font-size:1.35rem;box-shadow:none}.shop-nav{display:flex;align-items:center;justify-content:center;gap:20px;flex:1;overflow:auto}.shop-nav a,.shop-nav button{padding:8px 0;border:0;background:transparent;color:#101827;font-weight:650;white-space:nowrap}.shop-nav a:hover,.shop-nav button:hover{color:#078c83;box-shadow:none;transform:none}.shop-nav .print-link{color:#078c83;font-weight:900}.shop-menu{display:none}.cart{width:auto!important;margin:0!important;padding:11px 15px!important;color:#111827!important;background:#f2f4f5!important;border:1px solid #e3e7e9!important}.cart span{display:inline-grid;place-items:center;min-width:21px;height:21px;margin-left:4px;border-radius:50%;background:#111827;color:#fff;font-size:.72rem}.wrap{max-width:1740px;padding:26px max(20px,calc((100vw - 1740px)/2))}.hero{min-height:80px;margin:0 0 22px;padding:20px 24px;align-items:center;border:1px solid #e2e5e8;border-radius:18px;background:#fff}.hero h1{font-size:1.4rem;letter-spacing:-.04em}.hero .eyebrow{color:#159d92}.hero p{color:#5f6c78}.filters{margin:0 0 18px}.filters input{max-width:350px;border-color:#d8dde1;background:#fff;color:#111827}.filters select{display:none}.products{grid-template-columns:repeat(4,minmax(0,1fr));gap:22px}.product{overflow:hidden;background:#fff;color:#142033;border:1px solid #e0e5e8;border-radius:20px;box-shadow:none;transition:transform .24s ease,box-shadow .24s ease}.product:hover{transform:translateY(-7px);border-color:#d5dadd;box-shadow:0 18px 36px #12203018}.product .image,.product:has(.image img) .image{height:390px;background:#f4f5f6}.product .image img{object-fit:cover;object-position:center}.product:has(.image img){min-height:0;background:#fff}.product .info,.product:has(.image img) .info{position:relative;inset:auto;min-height:210px;padding:23px 26px 20px;border:0;border-radius:0;background:#fff;backdrop-filter:none;box-shadow:none;text-shadow:none}.product .tag,.product:has(.image img) .tag{color:#6b7280;font-size:.67rem;font-weight:800}.product h2,.product:has(.image img) h2{min-height:4.05em;margin:9px 0;color:#101827;font-size:1.15rem;line-height:1.35}.product .price,.product:has(.image img) .price{display:flex;align-items:baseline;gap:9px;flex-wrap:wrap;color:#e11d48;font-size:1.08rem}.product .price:has(.old)::before{content:'Prix réduit';font-size:.64rem;font-weight:850;letter-spacing:.06em;text-transform:uppercase}.product .price:not(:has(.old))::before{content:'Prix normal';font-size:.64rem;font-weight:850;letter-spacing:.06em;text-transform:uppercase;color:#64748b}.product .old,.product:has(.image img) .old{color:#8b96a1;font-size:.83rem}.product .old::before{content:'Avant · '}.product .stock,.product:has(.image img) .stock{color:#607080}.product .info button{position:absolute;right:18px;top:-58px;width:auto;margin:0;padding:12px 14px;background:#111827;border-radius:10px}.product .info button:hover{background:#1f2937}.empty{grid-column:1/-1;background:#fff}@keyframes shopEnter{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}.product{animation:shopEnter .48s both}.product:nth-child(2){animation-delay:.05s}.product:nth-child(3){animation-delay:.1s}.product:nth-child(4){animation-delay:.15s}@media(max-width:980px){.shop-nav{gap:13px}.products{grid-template-columns:repeat(3,minmax(0,1fr))}.product .image,.product:has(.image img) .image{height:300px}}@media(max-width:700px){body{background:#f5f6f7}.top{height:74px;padding:0 18px;gap:12px}.brand i{width:40px;height:40px}.shop-menu{display:block;width:auto;margin:0;padding:8px;background:transparent;color:#101827;font-size:1.5rem}.shop-nav{display:none;position:fixed;top:74px;left:12px;right:12px;z-index:20;max-height:70vh;padding:16px;overflow:auto;border:1px solid #e1e5e8;border-radius:16px;background:#fffffff5;box-shadow:0 20px 45px #12203027;flex-direction:column;align-items:stretch}.shop-menu-open .shop-nav{display:flex;animation:shopEnter .2s ease}.shop-nav a,.shop-nav button{text-align:left;padding:11px}.cart{padding:9px 11px!important}.wrap{padding:14px 12px 40px}.hero{display:none}.filters input{max-width:none;width:100%;padding:12px}.products{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.product{border-radius:17px}.product .image,.product:has(.image img) .image{height:205px}.product .info,.product:has(.image img) .info{min-height:215px;padding:16px 14px}.product h2,.product:has(.image img) h2{font-size:1rem;min-height:4.1em}.product .price{font-size:1rem}.product .info button{right:10px;top:-49px;padding:10px;font-size:0}.product .info button:after{content:'🛒';font-size:1rem}@media(prefers-reduced-motion:reduce){.product,.shop-menu-open .shop-nav{animation:none;transition:none}}}
    """
    layout="""(()=>{const header=document.querySelector('.top');if(!header)return;header.innerHTML='<button class="shop-menu" id="shopMenu" aria-label="Ouvrir le menu">☰</button><a class="brand" href="/"><i>F</i>FUSAA</a><nav class="shop-nav" id="shopNav"><a href="/">Accueil</a><a class="print-link" href="/impression">▣ Impression</a><button type="button" data-shop-all="true">Boutique</button></nav><button id="cartButton" class="cart">Panier <span id="cartCount">0</span></button>';document.getElementById('shopMenu').onclick=()=>document.body.classList.toggle('shop-menu-open');const category=document.getElementById('category'),nav=document.getElementById('shopNav');const sync=()=>{if(!category)return;nav.querySelectorAll('[data-shop-category]').forEach(node=>node.remove());[...category.options].slice(1).forEach(option=>{const button=document.createElement('button');button.type='button';button.dataset.shopCategory=option.value;button.textContent=option.textContent;button.onclick=()=>{category.value=option.value;category.dispatchEvent(new Event('change'));document.body.classList.remove('shop-menu-open')};nav.append(button)});nav.querySelector('[data-shop-all]').onclick=()=>{category.value='';category.dispatchEvent(new Event('change'))}};if(category){new MutationObserver(sync).observe(category,{childList:true});setTimeout(sync,0)}})();"""
    return "document.head.insertAdjacentHTML('beforeend',"+json.dumps("<style>"+css+"</style>")+ ");\n"+layout+"\n"
...
def shop_js():
    ...
    layout_fixes=".dialog .line{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:10px}.cart-quantity{display:inline-flex;align-items:center;gap:5px;white-space:nowrap}.cart-quantity .cart{display:inline-grid;place-items:center;width:28px!important;height:28px;padding:0!important;border-radius:8px!important}.cart-quantity b{min-width:18px;text-align:center}.dialog .line>span:last-child{white-space:nowrap;text-align:right}.dialog .line>span:last-child>.cart{display:inline-grid;place-items:center;width:28px!important;height:28px;padding:0!important;border-radius:8px!important}@media(max-width:1100px){.products{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:760px){.products{grid-template-columns:repeat(2,minmax(0,1fr))}.product .image,.product:has(.image img) .image{height:210px}}@media(max-width:520px){.dialog{padding:18px 14px;border-radius:20px}.dialog .line{grid-template-columns:minmax(0,1fr) auto;gap:7px}.dialog .line>span:first-child{font-size:.78rem}.cart-quantity{grid-column:1;grid-row:2}.dialog .line>span:last-child{grid-column:2;grid-row:1;font-size:.76rem}.product .info button{font-size:.75rem}}@media(max-width:470px){.product .image,.product:has(.image img) .image{height:190px}}"
    layout_fixes += ".top{width:100%!important}.wrap{width:100%!important;max-width:1740px!important;margin-left:auto!important;margin-right:auto!important}.filters{display:grid;grid-template-columns:minmax(0,1fr) minmax(220px,300px);gap:10px}.filters input{max-width:none!important}.filters select{display:block!important;width:100%;border-color:#d8dde1!important;background:#fff!important;color:#111827!important}@media(min-width:1101px){.products{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:620px){.filters{grid-template-columns:1fr}.filters select{padding:12px}}"
    theme_css=":root[data-theme=dark]{color-scheme:dark;background:#07111f;color:#eaf5ff}:root[data-theme=dark] body{background:radial-gradient(circle at 86% -10%,#2876f544,transparent 30rem),#07111f}:root[data-theme=dark] .top{background:#07111fec!important;color:#eaf5ff!important;border-color:#ffffff13}:root[data-theme=dark] .shop-nav a,:root[data-theme=dark] .shop-nav button{color:#d9e9f5}:root[data-theme=dark] .cart{color:#eaf5ff!important;background:#ffffff0e!important;border-color:#ffffff1b!important}:root[data-theme=dark] .cart span{background:#eaf5ff;color:#07111f}:root[data-theme=dark] .hero,:root[data-theme=dark] .product{background:linear-gradient(145deg,#10273ddb,#0a1725ee)!important;color:#eaf5ff!important;border-color:#ffffff14}:root[data-theme=dark] .hero h1,:root[data-theme=dark] .product h2{color:#fff!important}:root[data-theme=dark] .hero p,:root[data-theme=dark] .product .stock{color:#a8c0d0!important}:root[data-theme=dark] .filters input,:root[data-theme=dark] .filters select{background:#0a1a2a!important;color:#eaf5ff!important;border-color:#31546c!important}:root[data-theme=dark] .product .image{background:#102a3d!important}:root[data-theme=dark] .product .info{background:#0a1725!important}:root[data-theme=dark] .shop-theme{width:auto!important;margin:0!important;padding:10px 12px!important}@media(max-width:700px){:root[data-theme=dark] .shop-nav{background:#0c1c2af5}:root[data-theme=dark] .shop-menu{color:#eaf5ff}}"
    # JSON encoding keeps CSS quotes (for example the sale-price labels) safe
    # inside the JavaScript response.
    theme_css += ":root[data-theme=light]{color-scheme:light;background:#f4f7fa;color:#15263a}:root[data-theme=light] body{background:radial-gradient(circle at 88% -8%,#c7f3ed88,transparent 30rem),linear-gradient(180deg,#f8fbfd,#edf3f7)!important;color:#15263a}:root[data-theme=light] .wrap{background:transparent}:root[data-theme=light] .top{background:#ffffffef!important;color:#102033!important;border-color:#dbe5ea}:root[data-theme=light] .hero,:root[data-theme=light] .product,:root[data-theme=light] .empty{background:#fff!important;color:#142033!important;border-color:#dce6ed}:root[data-theme=light] .hero h1,:root[data-theme=light] .product h2{color:#102033!important}:root[data-theme=light] .hero p,:root[data-theme=light] .product .stock{color:#566f81!important}:root[data-theme=light] .filters input,:root[data-theme=light] .filters select{background:#fff!important;color:#142033!important;border-color:#b9cbd6!important}:root[data-theme=light] .shop-ai{color:#15263a;background:linear-gradient(145deg,#ffffff,#eaf6f7)}:root[data-theme=light] .shop-ai-answer,:root[data-theme=light] .payment-guide{color:#234156;background:#edf8f7;border-color:#9edfd4}:root[data-theme=light] .shop-ai-close{color:#234156;background:#e8f0f4}:root[data-theme=light] .shop-ai-chips button{color:#234156;background:#e8f1f5}:root[data-theme=light] .dialog{color:#15263a;background:#fff;border-color:#8edfd2}:root[data-theme=light] .field,:root[data-theme=light] .notice{color:#4a6578}:root[data-theme=light] .field input,:root[data-theme=light] .field textarea,:root[data-theme=light] .field select{color:#15263a;background:#fff;border-color:#b9cbd6}"
    bootstrap="document.head.insertAdjacentHTML('beforeend',"+json.dumps("<style>"+glass_css+layout_fixes+theme_css+"</style>")+ ");\n"
    bootstrap += "(()=>{const apply=mode=>{const theme=mode==='light'?'light':'dark';document.documentElement.dataset.theme=theme;localStorage.setItem('fusaa-theme',theme);const button=document.getElementById('shopTheme');if(button)button.textContent=theme==='light'?'☾ Sombre':'☀ Clair'};window.toggleShopTheme=()=>apply(document.documentElement.dataset.theme==='light'?'dark':'light');const cart=document.getElementById('cartButton');if(cart&&!document.getElementById('shopTheme'))cart.insertAdjacentHTML('beforebegin','<button id=\"shopTheme\" class=\"cart shop-theme\" type=\"button\" onclick=\"toggleShopTheme()\"></button>');apply(localStorage.getItem('fusaa-theme')||'dark')})();\n"
    bootstrap += "(()=>{const nav=document.getElementById('shopNav');if(!nav)return;const removeCategories=()=>nav.querySelectorAll('[data-shop-category]').forEach(node=>node.remove());new MutationObserver(removeCategories).observe(nav,{childList:true});removeCategories()})();\n"
    return HTMLResponse(storefront_layout_script()+bootstrap+(Path(__file__).parent/"web"/"shop.js").read_text(encoding="utf-8"),media_type="application/javascript",headers={"Cache-Control":"no-store, max-age=0"})
