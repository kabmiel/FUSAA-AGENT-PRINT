...
def shop_js():
    glass_css=".product{position:relative}.product:has(.image img){min-height:355px;background:#102a3d}.product:has(.image img) .image{height:355px}.product:has(.image img) .info{position:absolute;z-index:1;left:10px;right:10px;bottom:10px;padding:13px;border:1px solid #ffffff42;border-radius:15px;background:linear-gradient(135deg,#061827df,#102b3bdd);backdrop-filter:blur(14px) saturate(145%);box-shadow:0 10px 28px #0009;text-shadow:0 1px 2px #000}.product:has(.image img) .tag{color:#64f0d6;font-weight:900}.product:has(.image img) h2,.product:has(.image img) .price{color:#fff}.product:has(.image img) .stock{color:#d4e5ed}.product:has(.image img) .old{color:#d1e0ea}@media(max-width:470px){.product:has(.image img),.product:has(.image img) .image{min-height:330px;height:330px}}"
