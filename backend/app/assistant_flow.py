...
def safe_result_response(calls: list[ToolCall]):
    ...
    if not calls:
        return AssistantResponse(title="Assistant FUSAA", answer="Je peux vous aider à retrouver un document, un travail ou une imprimante.", steps=["Essayez : « Quels travaux sont en attente ? » ou « Quelles imprimantes sont disponibles ? »"], tool_calls=[], requires_confirmation=False)
    call = calls[0]
...
def workflow_response(data, db, user):
    ...
    if re.search(r"\b(prix|tarifs?|cout|devis|combien)\b", text):
        ...
        if not ids:
            return AssistantResponse(title="Tarifs FUSAA", answer="Votre rôle permet de consulter les travaux, mais pas les tarifs de l’atelier.", tool_calls=[], requires_confirmation=False)
        org_ids = [item.organization_id for item in db.query(Workshop).filter(Workshop.id.in_(ids)).all()]
        rule = db.query(PriceRule).filter(PriceRule.organization_id.in_(org_ids), PriceRule.name == "Tarif public par défaut").order_by(PriceRule.priority).first()
        pricing = (rule.pricing if rule else {}) or {}

        def rate(prefix):
            standard = float(pricing.get(f"{prefix}_page") or pricing.get("per_page", 0) or 0)
            threshold = int(pricing.get(f"{prefix}_discount_from", 0) or 0)
            reduced = float(pricing.get(f"{prefix}_discount_page", standard) or standard)
            return standard, threshold, reduced

        mono, color = rate("monochrome"), rate("color")

        def line(label, values):
            standard, threshold, reduced = values
            return f"{label} : {standard:,.0f} FCFA/page" + (f" ; {reduced:,.0f} FCFA/page à partir de {threshold} pages" if threshold else "")

        amount_match = re.search(r"\b(\d+)\s*(?:page|pages|feuille|feuilles|copie|copies|impression|impressions)\b", text)
        wants_color = "couleur" in text or "color" in text
        wants_mono = any(word in text for word in ("noir", "blanc", "mono"))
        steps = [line("Noir et blanc", mono), line("Couleur", color)]
        if amount_match and (wants_color or wants_mono):
            sheets = int(amount_match.group(1))
            chosen = color if wants_color else mono
            unit = chosen[2] if chosen[1] and sheets >= chosen[1] else chosen[0]
            mode = "couleur" if wants_color else "noir et blanc"
            return AssistantResponse(title="Estimation FUSAA", answer=f"Pour {sheets} pages en {mode} : {unit * sheets:,.0f} FCFA.", steps=[f"Tarif appliqué : {unit:,.0f} FCFA par page", f"Quantité : {sheets} pages", *steps], next_view="publicOrders", next_label="Configurer les tarifs", tool_calls=[], requires_confirmation=False)
        return AssistantResponse(title="Tarifs FUSAA", answer="Voici la grille de prix actuellement configurée.", steps=steps + ["Dites par exemple : « combien pour 20 pages noir et blanc ? »"], next_view="publicOrders", next_label="Configurer les tarifs", tool_calls=[], requires_confirmation=False)
    ...
    if not re.search(r"imprim|prepar|annul|ignorer", text):
        return None
    if re.search(r"imprimante", text) and not re.search(r"imprimer|imprime\b|prepar|annul", text):
        return None
    ...
    if printer:
        details.append(f"Imprimante : {printer.name}")
